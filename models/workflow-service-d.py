...
import time
import subprocess
from model import checkerInfer, r2_d_predictor, fc_model

CURRENT_TRAFFIC = "103"
CURRENT_TRAFFIC_V2 = "113"
FC_MODEL_DIR = "models/dl/r1_service_d_fc"
LSTM_MODEL_DIR = "models/dl/r1_service_d_lstm"

def init_vegeta():
    command = "echo GET \"http://localhost:10000/request1\" |" \
        "vegeta attack -rate=" + CURRENT_TRAFFIC + " -duration=0 | vegeta report > serviced.txt"
    subprocess.call(command, shell=True)

def init_vegeta_v2():
    command = "echo GET \"http://localhost:10000/request2\" |" \
        "vegeta attack -rate=" + CURRENT_TRAFFIC_V2 + " -duration=0 | vegeta report > serviced2.txt"
    subprocess.call(command, shell=True)
...
def worker():
    ...
    time.sleep(1)
    r1_throughput = get_throughput('./serviced.txt')
    r2_throughput = get_throughput('./serviced2.txt')
    ...
    current_stat["mean_latency"] = get_mean_latency('./serviced.txt')

    current_stat_val = [float(i) for i in list(current_stat.values())]

    new_limit = r2_d_predictor(current_stat_val)
