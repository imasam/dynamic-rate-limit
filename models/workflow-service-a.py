...
import time
import subprocess
from model import checkerInfer, r1_a_predictor, fc_model, lstm_model

CURRENT_TRAFFIC = "100"
CURRENT_TRAFFIC_V2 = "110"
FC_MODEL_DIR = "models/dl/r1_service_a_fc"
LSTM_MODEL_DIR = "models/dl/r1_service_a_lstm"

def init_vegeta():
    command = "echo GET \"http://localhost:10000/request1\" |" \
        "vegeta attack -rate=" + CURRENT_TRAFFIC + " -duration=0 | vegeta report > servicea.txt"
    subprocess.call(command, shell=True)

def init_vegeta_v2():
    command = "echo GET \"http://localhost:10000/request2\" |" \
        "vegeta attack -rate=" + CURRENT_TRAFFIC_V2 + " -duration=0 | vegeta report > servicea2.txt"
    subprocess.call(command, shell=True)


# define query dict
CURRENT_REQUEST_SERVICE = "r1_service_a"
...
def worker():
    ...
    time.sleep(1)
    r1_throughput = get_throughput('./servicea.txt')
    r2_throughput = get_throughput('./servicea2.txt')
    ...
    current_stat["mean_latency"] = get_mean_latency('./servicea.txt')

    current_stat_val = [float(i) for i in list(current_stat.values())]

    new_limit = r1_a_predictor(current_stat_val)
